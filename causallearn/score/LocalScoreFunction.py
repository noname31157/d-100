...
# Newly Designed Score Function for single edge

def single_edge_score_BIC(Data: ndarray, i: int, PAi: List[int], parameters=None) -> float:
    ...
    #     # Extract the column corresponding to the target variable
    #     selected_data = Data[:, i]
        
    #     # Calculate the mean and variance for the variable
    #     mean_value = np.mean(selected_data)
    #     variance_value = np.var(selected_data)
        
    #     # Create a normal distribution object for the variable
    #     likelihood_distribution = multivariate_normal(mean=mean_value, cov=variance_value, allow_singular=True)                
        
    #     # Calculate log-likelihood for each data point
    #     log_likelihood_values = likelihood_distribution.logpdf(selected_data)    

    #     local_score = np.sum(log_likelihood_values)
    
    #     return local_score

    # Extract the relevant columns from the dataset
    selected_columns = PAi + [i]
    #print(selected_columns)
    selected_data = Data[:, selected_columns]

    # Calculate the mean vector and covariance matrix for the conditional distribution
    mean_vector = np.mean(selected_data, axis=0)
    covariance_matrix = np.cov(selected_data, rowvar=False)

    # Create a multivariate normal distribution object for the conditional likelihood --> allow_singular=True for handling singualr matrices
    conditional_likelihood = multivariate_normal(mean=mean_vector, cov=covariance_matrix, allow_singular=True)

    # Calculate log-likelihood for each data point
    conditional_log_likelihood = conditional_likelihood.logpdf(selected_data)
    ...
    local_score = np.sum(conditional_log_likelihood) - lambda_value * log(n) * 1   #Subtract the penalty if LL is -ve and vice versa
    
    return local_score

# Newly Designed Score Function
...
#     # Extract the relevant columns from the dataset
#     selected_columns = np.append(PAi, i)
#     #print(selected_columns)
#     selected_data = Data[:, selected_columns]
...
#     local_score = np.sum(conditional_log_likelihood) - lambda_value * log(n) * len(PAi)   #Subtract the penalty if LL is -ve and vice versa
...
#     x_var = n * H
#     y_var = np.log(n) * len(PAi) * lambda_value

#     local_score = x_var + y_var
...
def local_score_BIC(Data: ndarray, i: int, PAi: List[int], parameters=None) -> float:
    ...
    if len(PAi) == 0:
        z_var = n * np.log(cov[i, i])
        file_path="BICscore.csv"
        #print("=======================>" + str(z_var))
        write_to_csv([i, z_var, -1], file_path)
        return z_var
    ...
    file_path="BICscore.csv"
    write_to_csv([i, x_var[0,0], y_var], file_path)
