...
def insert_validity_test2(G, i, j, T) -> int:
    ...
    s2 = insert_vc2_new(G, j, i, np.union1d(NA, T), max_stack_size=25) #use this when using the newly designed function
    if s2:
        V = 1
...
#---------------MY NEWLY DESIGNED insert_vc2_new FUNCTION-----------------


def insert_vc2_new(G, j, i, NAT, max_stack_size=25):
    
    start = j
    target = i
    NAT = set(NAT)
    
    accessible = {idx: set(np.where(G.graph[:, idx] == Endpoint.ARROW.value)[0]) |
                  set(np.intersect1d(np.where(G.graph[idx, :] == Endpoint.TAIL.value)[0],
                                     np.where(G.graph[:, idx] == Endpoint.TAIL.value)[0]))
                  for idx in range(len(G.graph))}
    ...
    while stack:
        ...
        if current_node == target:
            ...
            next_nodes = accessible[current_node] - path_set
            
            for next_node in next_nodes:

                # Check stack size before appending
                if len(stack) >= max_stack_size:
                    break
...
# def insert_vc2_new(G, j, i, NAT, max_depth=100, max_stack_size=25, max_total_time=20):
...
#     stack = [(start, [])]
#     sign = 1
#     time_exceeded = False  # Flag to indicate if the time limit was exceeded
    
#     while stack:
#         # if time.time() - start_time > max_total_time:
#         #     print(f"Total execution time exceeded {max_total_time} seconds. Terminating search.")
#         #     time_exceeded = True
#         #     break
        
#         if len(stack) > max_stack_size:
#             #print(f"Stack size limit of {max_stack_size} exceeded.")
#             break
...
#             if len(stack) >= max_stack_size:
#                     #print(f"Stack size limit of {max_stack_size} exceeded while appending.")
#                     time_exceeded = True
#                     break
...
#                 # # Check if the total execution time exceeds the limit
#                 # if time.time() - start_time > max_total_time:
#                 #     print(f"Total execution time exceeded {max_total_time} seconds. Terminating search.")
#                 #     time_exceeded = True
#                 #     break
...
#             # # Break the while loop if time exceeded
#             # if time_exceeded:
#             #     break
...
# def insert_vc2_new(G, j, i, NAT, max_depth=100, max_stack_size=1000, max_total_time=20):
#     start_time = time.time()
#     tracemalloc.start()
...
#                 #print("Yes inside the for loop with i and j", i, j)
#                 stack.append((next_node, path + [current_node]))
...
# def insert_vc2_new(G, j, i, NAT):
#     """
#     Validity test for condition 2 of Insert operator.
#     G: CPDAG
#     j: Starting node
#     i: Target node
#     NAT: Set of nodes (NA ∪ T)
#     """
...
#     # Precompute accessible nodes from each node
#     accessible = dict((idx, set(np.where(G.graph[:, idx] == Endpoint.ARROW.value)[0]) |
#                             set(np.intersect1d(np.where(G.graph[idx, :] == Endpoint.TAIL.value)[0],
#                                                np.where(G.graph[:, idx] == Endpoint.TAIL.value)[0])))
#                       for idx in range(len(G.graph)))

#     # Stack for DFS; each element is (current_node, path)
#     stack = [(start, [])]
#     sign = 1
...
#         # Ensure path is a list and convert to set
#         path_set = set(path)

#         # # Debug prints to verify types
#         # print(f"Current Node: {current_node}")
#         # print(f"Path Set: {path_set}")
#         # print(f"NAT: {NAT}")

#         # Check if we've reached the target node
#         if current_node == target:
#             if not path_set & NAT:  # No intersection with NAT
#                 sign = 0
#                 break
#         else:
#             # # Get the next nodes to visit, excluding already visited ones
#             # next_nodes = accessible[current_node] - path_set
#             # for next_node in next_nodes:
#             #     stack.append((next_node, path + [current_node]))
...
#             for next_node in next_nodes:
#                 stack.append((next_node, path + [current_node]))
#             path_set.add(current_node)  # Mark the current node as visited
